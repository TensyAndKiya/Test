...
public class JAVA8DateTest {
    ...
    public static void main(String[] args) {
        ...
        PrintUtil.dateLine(now.toEpochMilli());
        ...
        PrintUtil.dateLine(diff_mills);

        PrintUtil.dateLine((System.currentTimeMillis() + 8 * ONE_HOUR_MILLS) % ONE_DAY_MILLS / ONE_HOUR_MILLS);
        //介绍
        //以下都是不可变类型且线程安全的哦
        //Instant 代表时间戳
    ...
    private static void show(LocalDate localDate) {
        PrintUtil.dateLine("今天的日期是：" + localDate);
        PrintUtil.dateLine("是不是闰年：" + localDate.isLeapYear());
        //日期的加减 ChronoUnit单位  那个参数可以用负数哦
        PrintUtil.dateLine("一周后的日期是：" + localDate.plus(1, ChronoUnit.WEEKS));
        PrintUtil.dateLine("一周后的日期是：" + localDate.plusWeeks(1));
        PrintUtil.dateLine("一年前的日期是：" + localDate.minus(1, ChronoUnit.YEARS));
        PrintUtil.dateLine("一年前的日期是：" + localDate.minusYears(1));

        int year = localDate.getYear();
        int month = localDate.getMonthValue();
        int day = localDate.getDayOfMonth();
        PrintUtil.dateLine("今天是" + year + '年' + month + '月' + day + '日');
        //获取特定日期类型的LocalDate
        //year 是 0到9999 10000或以上会多个+号，month是1到12，day是1到31(但会根据前面的month判定是否合法) 输入错误数字会报错
        LocalDate localDate2 = LocalDate.of(1995, 10, 3);
        PrintUtil.dateLine("你输入的日期是：" + localDate2);
        //判断日期是否相等 或者用isEqual方法
        PrintUtil.dateLine(localDate.equals(localDate2));
        PrintUtil.dateLine(LocalDate.of(1995, 10, 03).equals(localDate2));
        ...
        if (theDay.equals(birthDay)) {
            PrintUtil.dateLine("是你的生日哦！");
        }else{
            PrintUtil.dateLine("不是你的哦！");
        }
    }

    private static void show(LocalTime localTime) {
        PrintUtil.dateLine("当前的时间是：" + localTime);
        PrintUtil.dateLine("两小时后的时间是：" + localTime.plusHours(2));
    }

    private static void show(LocalDateTime localDateTime) {
        PrintUtil.dateLine("当前的日期时间是：" + localDateTime);
        //这个有点问题。。时钟不对。。忘了+8了吗。。。
        //好像又没问题了。。因为算的是从(计算机元年)1970 01 01 00 00 00 到现在的时间戳。。
        PrintUtil.dateLine("当前的时间戳是：" + Instant.now());
        PrintUtil.dateLine("当前的时间戳是：" + Instant.now(Clock.systemUTC()));
        PrintUtil.dateLine("当前的时间戳是：" + Clock.systemDefaultZone().instant());
        PrintUtil.dateLine("当前的时间戳是：" + Clock.system(ZoneId.of(ZoneId.SHORT_IDS.get("CTT"))).instant());
        //将本地时间转成另一个时区的时间
        ZoneId zoneId = ZoneId.of(ZoneId.SHORT_IDS.get("CTT"));
        ZonedDateTime dateTimeInNewYork = ZonedDateTime.of(localDateTime, zoneId);
        PrintUtil.dateLine("现在是上海时间：" + dateTimeInNewYork);
    }

    private static void showClock(){
        //根据系统时钟和UTC返回当前时间
        Clock clock = Clock.systemUTC();
        PrintUtil.dateLine("clock: " + clock);
    ...
    private static void dateTimeFormat() {
        ...
        PrintUtil.dateLine("parse后的时间为：" + localDate);
        //format
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss:SSS");
        LocalDateTime localDateTime = LocalDateTime.now();
        PrintUtil.dateLine("格式化后的日期时间为：" + dtf.format(localDateTime));
    }
}
